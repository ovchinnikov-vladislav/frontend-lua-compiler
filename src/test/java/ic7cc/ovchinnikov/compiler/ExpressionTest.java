...
public class ExpressionTest {
    ...
    public void expressionTest() throws Exception {
        File file = Path.of("lua/expression/expression.lua").toFile();

        Parser parser = new Parser(new Lexer(new FileReader(file)));
        BlockNode block = (BlockNode) parser.parse().value;

        xmlSerializer.save(block, Path.of("result/expression/xml/expression.xml").toFile());

        HashMap<String, Object> result = xmlSerializer.read(Path.of("result/expression/xml/expression.xml").toFile());
        HashMap<String, Object> expected = xmlSerializer.read(Path.of("expected/expression/xml/expected_expression.xml").toFile());
        ...
        Assert.assertEquals(ExpressionDataTypeTableBuilder.Type.BOOLEAN, map.get("a"));
    }
